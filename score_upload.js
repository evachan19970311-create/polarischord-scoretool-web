(async function () {
  ...
  const CORE_SCRIPT_URL =
    'https://cdn.jsdelivr.net/gh/evachan19970311-create/polarischord-scoretool-web@847c84ab70fd7eee391cb325c28060b988fbac29/score_upload.js?v=basic-course-dialog-prod-20260601';
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`score_upload.js の本体取得に失敗しました (${response.status})`);
    }
    ...
    core_script = core_script
      .replace(
        ...
        "const request_json = ($, url, data) => {\n    const request_url = new URL(url, location.href).toString();\n\n    return new Promise((resolve, reject) => {\n      $.ajax({\n        url,\n        type: 'POST',\n        dataType: 'json',\n        data\n      })\n        .done((res) => resolve(res))\n        .fail((xhr) => {\n          if (window.__score_upload_is_basic_course_like_error__ && window.__score_upload_is_basic_course_like_error__(xhr)) {\n            reject(window.__score_upload_build_basic_course_error__(xhr, request_url));\n            return;\n          }\n\n          reject(xhr);\n        });\n    });\n  };"
      )
      .replace(
      ...
      .replace(
        ...
        "if (error?.code === 'BASIC_COURSE_REQUIRED') {\n        window.__score_upload_hide_status_card__?.();\n        result.style.display = 'none';\n        window.__score_upload_show_basic_course_dialog__?.();\n      } else {\n        result.textContent = (error.message || String(error)) + stageText;\n      }"
      );

    window.__score_upload_is_basic_course_like_error__ = is_basic_course_like_error;
    window.__score_upload_build_basic_course_error__ = build_basic_course_error;
    window.__score_upload_show_basic_course_dialog__ = show_basic_course_dialog;
    window.__score_upload_hide_status_card__ = hide_score_upload_status_card;

    const script = document.createElement('script');
    script.textContent = `${core_script}\n//# sourceURL=score_upload_basic_course_check_core.js`;
